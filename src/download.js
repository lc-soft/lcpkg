...
class Downloader {
  ...
  getTaskName(filename) {
    return filename.padEnd(this.files.reduce((len, file) => Math.max(file.length, len), 0), ' ');
  }
...
async function downloadSourcePackage(downloader, {
  name, version, uri, sourcePath
}) {
